...
const AIInsightCard: React.FC<{ doc: any }> = ({ doc }) => {
  ...
  return (
    <>
        <GlassCard className="flex flex-col md:flex-row justify-between items-center gap-4 group hover:border-teal/30 transition-all">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-xs text-text-secondary">{new Date(doc.date).toLocaleDateString()} • {doc.type}</p>
                </div>
            </div>
        ...
        {/* Insight Modal */}
        <Modal isOpen={open} onClose={() => setOpen(false)} title="Clinical AI Analysis">
            {ai && (
                <div className="space-y-6">
                    <div className="p-4 bg-teal/5 border border-teal/20 rounded-lg">
                        <h4 className="text-teal font-bold mb-2 flex items-center gap-2">
                             <FileText size={16} /> Automated Summary
                        </h4>
                    ...
                    {ai.highlights && ai.highlights.length > 0 && (
                        <div>
                            <h5 className="text-xs text-text-secondary uppercase tracking-wider mb-2">Key Medical Terms</h5>
                    ...
                    {ai.recommendations && (
                        <div className="space-y-2">
                             <h5 className="text-xs text-text-secondary uppercase tracking-wider">Suggested Actions</h5>
                    ...
                    <div className="pt-4 border-t border-white/10 flex flex-col gap-2">
                        <div className="flex items-center gap-2 text-[10px] text-text-secondary">
                            <ShieldCheck size={12} className="text-teal"/> Analysis Engine: {ai.source}
                        </div>
                        <div className="bg-black/40 border border-white/5 p-3 rounded-lg flex items-start gap-3">
                             <AlertCircle className="text-text-secondary flex-shrink-0" size={16} />
                             <p className="text-[10px] text-text-secondary leading-relaxed">
                                <strong>Regulatory Disclaimer:</strong> This summary is generated by an AI Clinical Decision Support System. 
                                It is designed to assist in data interpretation but <strong>does not constitute a medical diagnosis</strong>. 
                                All findings must be verified by a licensed healthcare professional. 
                                Wysh Care complies with data retention and audit standards.
                             </p>
                        </div>
                    </div>
                </div>
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
        <div className="flex justify-between items-center">
            <div>
                 ...
                 <p className="text-text-secondary text-sm">Automated analysis of your medical history with clinical guardrails.</p>
            </div>
        </div>

        {loading ? (
             <div className="flex justify-center py-20"><Loader text="Loading secure insights..." /></div>
        ) : docs.length === 0 ? (
             <div className="text-center py-16 border border-dashed border-white/10 rounded-xl">
                 <Brain className="mx-auto text-text-secondary mb-4 opacity-50" size={48} />
                 <p className="text-text-secondary">No eligible documents for AI analysis found.</p>
