...
export const ScheduleManager: React.FC = () => {
  ...
  const handleSave = () => {
    addNotification('success', 'Schedule updated successfully');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
         {DAYS.map(day => (
           <GlassCard key={day} className="flex flex-col gap-4">
             <div className="flex justify-between items-center">
               ...
               <button 
                  ...
                  className="text-xs text-teal hover:text-white transition-colors flex items-center gap-1"
               >
             ...
             <div className="space-y-2">
               {(schedule[day] && schedule[day].length > 0) ? (
                 schedule[day].map((slot, idx) => (
                   <div key={idx} className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/5 group">
