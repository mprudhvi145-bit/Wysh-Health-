...
const MOCK_PATIENT_DETAILS: ClinicalPatient = {
...
export const doctorService = {
  ...
  getAllDoctors: async (specialty?: Specialty, search?: string): Promise<Doctor[]> => {
    // Directory is public, usually mocked for demo performance
  ...
  searchPatients: async (query: string): Promise<ClinicalPatient[]> => {
    const mockResult = [MOCK_PATIENT_DETAILS].filter(p => !query || p.name.toLowerCase().includes(query.toLowerCase()));
  ...
  getPatientDetails: async (id: string): Promise<ClinicalPatient> => {
    if (config.dataMode === 'MOCK') return MOCK_PATIENT_DETAILS;
    ...
    try {
      ...
      console.warn("API Chart failed, using mock", e);
  ...
  createPrescription: async (payload: { patientId: string, medications: any[], notes: string }) => {
    ...
    try {
      ...
      console.warn("API Prescribe failed, using mock", e);
      return { success: true };
    }
  },
  ...
  orderLab: async (payload: { patientId: string, testName: string, category: string, priority: string }) => {
    ...
    try {
      ...
      console.warn("API Lab Order failed, using mock", e);
      return { success: true };
    }
  },
  ...
  createNote: async (payload: { patientId: string, type: string, subject: string, content: string, isPrivate: boolean }) => {
    ...
    try {
      ...
      console.warn("API Note failed, using mock", e);
      return { success: true };
    }
  }
};
